 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
    class KT_WP_Import extends WP_Importer
    {
        ...
        var $ajax_options_posttype = array();
        var $ajax_options_name     = array();
        var $ajax_main_content;
        var $menu_import;
        ...
        function process_posts()
        {
            ...
            foreach ( $this->posts as $post ) {
                ...
                if ( $this->ajax_main_content == 1 && !empty( $this->ajax_options_posttype ) && in_array( $post['post_type'], $this->ajax_options_posttype ) ) {
                    continue;
                }
                if ( $this->ajax_main_content != 1 && !empty( $this->ajax_options_posttype ) && $post['post_type'] != $this->ajax_options_posttype[0] ) {
                    continue;
                }
                if ( !empty( $this->ajax_options_name ) && !in_array( $post['post_name'], $this->ajax_options_name ) ) {
                    continue;
                }
        ...
        function process_menu_item( $item )
        {
            ...
            $_menu_item_xfn               = '';
            $_menu_item_url               = '';
            $_menu_item_megamenu_img_icon = '';


            foreach ( $item[ 'postmeta' ] as $meta ) {

                if ( !isset( $this->menu_import ) && $this->menu_import != 1 )
                    continue;
                $$meta['key'] = $meta[ 'value' ];


                if ( trim( $meta[ 'key' ] ) == '_menu_item_type' ) {
                    $_menu_item_type = $meta[ 'value' ];
                }
                if ( trim( $meta[ 'key' ] ) == '_vc_post_settings' ) {
                    $_vc_post_settings = $meta[ 'value' ];
                }
                if ( trim( $meta[ 'key' ] ) == '_menu_item_menu_item_parent' ) {
                    $_menu_item_menu_item_parent = $meta[ 'value' ];
                }
                if ( trim( $meta[ 'key' ] ) == '_menu_item_object_id' ) {
                    $_menu_item_object_id = $meta[ 'value' ];
                }
                if ( trim( $meta[ 'key' ] ) == '_menu_item_object' ) {
                    $_menu_item_object = $meta[ 'value' ];
                }
                if ( trim( $meta[ 'key' ] ) == '_menu_item_target' ) {
                    $_menu_item_target = $meta[ 'value' ];
                }
                if ( trim( $meta[ 'key' ] ) == '_menu_item_classes' ) {
                    $_menu_item_classes = $meta[ 'value' ];
                }
                if ( trim( $meta[ 'key' ] ) == '_menu_item_xfn' ) {
                    $_menu_item_xfn = $meta[ 'value' ];
                }
                if ( trim( $meta[ 'key' ] ) == '_menu_item_url' ) {
